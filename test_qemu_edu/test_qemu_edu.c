...
static void free_dma_work_fn(struct work_struct *work) {
	struct edu_device *edu_dev = container_of(work, struct edu_device, free_dma_work);
...
static irqreturn_t edu_irq_handler(int irq, void *dev_id) {
	...
	DRV_LOG_INFO("[%s] Receive interrupt, status: 0x%x\n", DRIVER_NAME, status);
	...
	if (edu_dev->user_pid && status == DMA_IRQ_VALUE) {
		...
		info.si_int = edu_dev->dma_direction & DMA_EDU2RAM ? DMA_EDU2RAM : DMA_RAM2EDU;
...
static ssize_t edu_read(struct file *filp, char __user *buf, size_t count,
	...
	case EDU_DMA_GET: {
		...
		if (copy_to_user((void __user *)edu_dev->dma_dst_address,
			...
			DRV_LOG_ERR("[%s] DMA GET: Failed to copy_to_user\n",
				    DRIVER_NAME);
			return -EFAULT;
		}
		DRV_LOG_INFO("[%s] DMA GET: Content: %s\n", DRIVER_NAME,
	...
	case EDU_DMA_CMD: {
		...
		if (cmd & DMA_EDU2RAM) {
			...
			if (edu_dev->dma_src_address < EDU_BUFFER_ADDRESS ||
			    edu_dev->dma_src_address + size >= EDU_BUFFER_ADDRESS + BUFFER_SIZE) {
				DRV_LOG_ERR("[%s] DMA_CMD: Memory out of bounds\n",
					    DRIVER_NAME);
			...
			if (edu_dev->dma_dst_address < EDU_BUFFER_ADDRESS ||
			    edu_dev->dma_dst_address + size >= EDU_BUFFER_ADDRESS + BUFFER_SIZE) {
				DRV_LOG_ERR("[%s] DMA_CMD: Memory out of bounds\n",
					    DRIVER_NAME);
				dma_free_coherent(dev, size, buffer_addr, dma_addr);
				return -EFAULT;
			...
			if (copy_from_user(buffer_addr,
					   ...
					   edu_dev->dma_count)) {
				DRV_LOG_ERR("[%s] DMA_CMD: Failed to copy_from_user\n",
					    DRIVER_NAME);
				dma_free_coherent(dev, size, buffer_addr, dma_addr);
				return -EFAULT;
			}
...
static int edu_probe(struct pci_dev *pdev, const struct pci_device_id *id) {
	...
	if (!edu_dev) {
		DRV_LOG_ERR("[%s] allocate edu_device failed. \n", DRIVER_NAME);
		return -ENOMEM;
	}
	edu_dev->pdev = pdev;
	...
	if ((ret = pci_enable_device(pdev)) < 0) {
		DRV_LOG_ERR("[%s] pci_enable_device failed. \n", DRIVER_NAME);
	...
	if ((ret = alloc_chrdev_region(&dev_num, BASEMINOR, DEVICE_COUNTS,
								   DRIVER_NAME)) < 0) {
		DRV_LOG_ERR("[%s] Failed to allocate char device region\n", DRIVER_NAME);
		goto disable_device;
	}
	...
	if (cdev_add(&edu_dev->cdev, edu_dev->dev_num, DEVICE_COUNTS)) {
		DRV_LOG_ERR("[%s] Failed to add cdev\n", DRIVER_NAME);
	...
	if (IS_ERR(edu_class)) {
		DRV_LOG_ERR("[%s] Failed to create class\n", DRIVER_NAME);
	...
	if (device_create(edu_class, NULL, edu_dev->dev_num, NULL, "edu") == NULL) {
		DRV_LOG_ERR("[%s] Failed to create device node\n", DRIVER_NAME);
		ret = -EINVAL;
		goto destroy_class;
	}
	...
	if ((ret = pci_request_region(pdev, BAR, DRIVER_NAME)) < 0) {
		DRV_LOG_ERR("[%s] pci_request_region failed. \n", DRIVER_NAME);
		goto destroy_device;
	}
	...
	if (!mmio_base) {
		DRV_LOG_ERR("[%s] Cannot iomap BAR\n", DRIVER_NAME);
		ret = -ENOMEM;
		goto release_region;
	}
	...
	if ((ret = request_irq(pdev->irq, edu_irq_handler, IRQF_SHARED, DRIVER_NAME,
						   edu_dev)) < 0) {
		DRV_LOG_ERR("[%s] Failed to request IRQ %d\n", DRIVER_NAME,
			   pdev->irq);
		goto unmap_bar;
	}
	...
	DRV_LOG_INFO("[%s] probe sucessfully.\n", DRIVER_NAME);
	return 0;
 ...
 * @remove: gets called whenever a device being handled by this driver is removed
 */
...
static int __init edu_init(void) {
	...
	if (ret < 0) {
		DRV_LOG_ERR("Init failed.\n");
		return ret;
	}

	DRV_LOG_INFO("Init sucessfully.\n");
	return ret;
}
